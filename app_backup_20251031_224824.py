...
def iniciar_simulado():
    try:
        ...
        # Salva na sessão sem usar cookies grandes
        session['simulado_questoes_ids'] = [q['id'] for q in questoes]
        ...
        return jsonify({
            ...
            "questoes": questoes,  # Envia todas as questões de uma vez
...
def get_questao(indice):
    try:
        # Busca a questão diretamente do banco usando o ID salvo na sessão
        ...
        if 0 <= indice < total_questoes:
            ...
            # Busca a questão específica pelo ID
            cursor.execute("SELECT * FROM questoes WHERE id = ?", (questoes_ids[indice],))
            questao_raw = cursor.fetchone()
            conn.close()
            
            if not questao_raw:
                return jsonify({"success": False, "error": "Questão não encontrada."}), 404
            
            questao_atual = dict(questao_raw)
            questao_atual['alternativas'] = json.loads(questao_raw['alternativas'])
        ...
        print(f"Erro em /api/simulado/questao: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
...
def responder_questao():
    try:
        ...
        # Busca a questão correta do banco
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM questoes WHERE id = ?", (questao_id,))
        questao_correta_raw = cursor.fetchone()
        conn.close()
            
        if not questao_correta_raw:
             return jsonify({"success": False, "error": "ID da questão não encontrado no banco."}), 404

        questao_correta = dict(questao_correta_raw)
        questao_correta['alternativas'] = json.loads(questao_correta_raw['alternativas'])
...
def finalizar_simulado():
    try:
        ...
        # Busca todas as questões do banco para calcular o desempenho
        conn = get_db()
        cursor = conn.cursor()
